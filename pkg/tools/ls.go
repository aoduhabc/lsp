...
func NewLsTool(root string) BaseTool {
	return &lsTool{root: root}
}
...
func (l *lsTool) Info() ToolInfo {
	return ToolInfo{
		...
		Parameters: map[string]any{
			"path": map[string]any{"type": "string", "description": "The path to the directory to list (defaults to current working directory)"},
...
func (l *lsTool) Run(ctx context.Context, call ToolCall) (ToolResponse, error) {
	...
	if l.root != "" && !isWithinRoot(l.root, searchAbs) {
		return NewTextErrorResponse("path is outside workspace root"), nil
	}

	if _, err := os.Stat(searchAbs); os.IsNotExist(err) {
		return NewTextErrorResponse(fmt.Sprintf("path does not exist: %s", searchAbs)), nil
	}

	files, truncated, err := listDirectory(searchAbs, params.Ignore, 1000)
	if err != nil {
		return ToolResponse{}, fmt.Errorf("error listing directory: %w", err)
	}

	tree := createFileTree(files)
	output := printTree(tree, searchAbs)
	if truncated {
	...
	return WithResponseMetadata(
		...
		LSResponseMetadata{NumberOfFiles: len(files), Truncated: truncated},
	), nil
}
...
func createFileTree(sortedPaths []string) []*TreeNode {
	...
	for _, path := range sortedPaths {
		...
		for i, part := range parts {
			...
			newNode := &TreeNode{Name: part, Path: currentPath, Type: nodeType, Children: []*TreeNode{}}
