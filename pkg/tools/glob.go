...
func NewGlobTool(root string) BaseTool {
...
func (g *globTool) Run(ctx context.Context, call ToolCall) (ToolResponse, error) {
	...
	files, truncated, err := globFiles(params.Pattern, searchAbs, 100)
	if err != nil {
	...
	return WithResponseMetadata(
		NewTextResponse(out),
		GlobResponseMetadata{NumberOfFiles: len(files), Truncated: truncated},
	), nil
}
